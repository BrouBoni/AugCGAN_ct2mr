...
def variational_ubo(model, real_A, real_B, steps, visualize=False, vis_name=None, vis_path=None,
    ...
    dequant = Variable(torch.zeros(*real_B.size()).uniform_(0, 1.).cuda())
    size = real_A.size()
    ...
    logvar = Variable(torch.zeros(size[0], model.opt.nlatent).fill_(math.log(0.01)).cuda(),
                      requires_grad=True)
    if logvar_B is None:
        logvar_B = Variable(torch.zeros(1, 1, size[2], size[3]).fill_(math.log(0.01)).cuda())
    ...
    for i in range(steps):
        ...
        ubo = (-log_prob + kld) + (size[2]*size[3]*1) * math.log(0.5)
        ubo_val_new = ubo.mean(0).data[0]
        kld_val = kld.mean(0).data[0]
        bpp = ubo.mean(0).data[0] / (size[2]*size[3]-1* math.log(2.))
        if compute_l1:
...
def visualize_data(opt, data, size, save_path):
    ...
    a = (vis_image.cpu()[0,0].numpy()+1)*4000
    with open(save_path, 'wb') as f:
            writer = png.Writer(width=a.shape[0], height=a.shape[1], bitdepth=16, greyscale=True)
