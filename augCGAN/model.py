...
class AugmentedCycleGAN(object):
    ...
    def __init__(self, opt, testing=False):
        ...
        if opt.crop:
            inputSize = opt.cropSize
        else:
            inputSize = opt.loadSize

        self.netE_B = networks.define_E(nlatent=opt.nlatent, input_nc=enc_input_nc,
                                        loadSize=inputSize, nef=opt.nef, norm='batch',
                                        gpu_ids=opt.gpu_ids)
        ...
        if not testing:
            num_params = 0
            with open("%s/nets.txt" % opt.expr_dir, 'w') as nets_f:
                num_params += networks.print_network(self.netG_A_B, nets_f)
                num_params += networks.print_network(self.netG_B_A, nets_f)
                num_params += networks.print_network(self.netD_A, nets_f)
                num_params += networks.print_network(self.netD_B, nets_f)
                num_params += networks.print_network(self.netD_z_B, nets_f)
                num_params += networks.print_network(self.netE_B, nets_f)
    ...
    def train_instance(self, real_A, real_B, prior_z_B):
        ...
        loss_D_post_z_B, loss_D_prior_z_B, pred_post_z_B, pred_prior_z_B = \
            discriminate_z(self.netD_z_B, self.criterionGAN, post_z_realB.detach(), prior_z_B)
        ...
        losses  = OrderedDict([('D_A', loss_D_A.data[0]), ('G_A', loss_G_A.data[0]), ('Cyc_A', loss_cycle_A.data[0]),
                               ...
                               ('P_t_A', pred_true_A[0][0].data.mean()), ('P_f_A', pred_fake_A[0][0].data.mean()),
                               ('P_t_B', pred_true_B[0][0].data.mean()), ('P_f_B', pred_fake_B[0][0].data.mean())])
        visuals = OrderedDict([('real_A', real_A.data), ('fake_B', fake_B.data), ('rec_A', rec_A.data),
                               ('real_B', real_B.data), ('fake_A', fake_A.data), ('rec_B', rec_B.data)])
    ...
    def synthetize(self, real_B):
        fake_A = self.netG_B_A.forward(real_B)
        visual = OrderedDict([('fake_A', fake_A.data)])
        return visual
