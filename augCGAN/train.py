...
def train_model():
    ...
    if opt.numpy_data:
        ...
        dev_cycle_loader = DataLoader(opt, subset='dev', unaligned=False, batchSize=25, drop_last=True)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        if epoch % opt.eval_B_freq == 0:
            ...
            if opt.model == 'cycle_gan':
                ...
                steps = 50
            dev_ubo_B, dev_bpp_B, dev_kld_B = eval_ubo_B(dev_dataset, model, steps, False, 'pred_B_%d' % epoch,
                                                         opt.vis_pred_B)
